  ...
  io.on("connection", (socket: Socket) => {
    ...
    socket.on(
      ...
      (data: {
        ...
        log.info(
          { socketId: socket.id, areaId, role, judgeId },
          "client joined area room",
        );
    ...
    socket.on(
      ...
      (data: {
        ...
        if (type === ScoreUpdateType.POINT) {
          ...
          if (corner === "red")
            state.match.warnings.red = Math.max(
              0,
              state.match.warnings.red + value,
            );
          else
            state.match.warnings.blue = Math.max(
              0,
              state.match.warnings.blue + value,
            );
        } else if (type === ScoreUpdateType.DEDUCTION) {
          if (corner === "red")
            state.match.deductions.red = Math.max(
              0,
              state.match.deductions.red + value,
            );
          else
            state.match.deductions.blue = Math.max(
              0,
              state.match.deductions.blue + value,
            );
        }
    ...
    socket.on(SocketEvent.DISCONNECT, () => {
      ...
      if (currentAreaId && currentRole === SocketRole.JUDGE && currentJudgeId) {
        ...
        if (state && state.judges[currentJudgeId]) {
          ...
          io.to(`area:${currentAreaId}`).emit(
            SocketEvent.JUDGES_UPDATE,
            state.judges,
          );
        }
      }
