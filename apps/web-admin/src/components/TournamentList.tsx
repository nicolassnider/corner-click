...
export default function TournamentList({ onSelect, onCreateNew }: Props) {
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {tournaments.map(t => (
            <div 
              key={t.id} 
              onClick={() => onSelect(t)}
              className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm hover:shadow-xl transition-shadow cursor-pointer"
            >
              ...
              <div className="border-t border-gray-100 pt-4 mt-4">
                <p className="text-sm font-semibold text-gray-500">{t.rings} Areas Setup</p>
              </div>
            </div>
          ))}
        </div>
