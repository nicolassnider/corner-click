...
export const CompetitorForm: React.FC<CompetitorFormProps> = ({
  ...
  useEffect(() => {
    if (initialData) {
      ...
      setFormData({
        categoryId:
          initialCategoryId ||
          (categories.some((c) => c.id === categoryId) ? categoryId : ""),
  ...
  const uniqueBelts = Array.from(
    new Set(categories.map((c) => c.beltLevel)),
  ).filter(Boolean);
    ...
    if (ageGroup) {
      const weightClasses =
        formData.gender === "MALE"
          ? ageGroup.maleWeights
          : ageGroup.femaleWeights;
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="bg-gray-50 p-4 rounded-md border border-gray-200">
          ...
          <p className="mt-1 text-xs text-gray-500">
            La categoría se asignará automáticamente al ingresar los datos
            físicos del competidor.
          </p>
        </div>
        ...
        <div className="flex items-center mt-2">
          <input
            ...
            onChange={(e) =>
              setFormData({ ...formData, isSeeded: e.target.checked })
            }
          />
          <label
            htmlFor="isSeeded"
