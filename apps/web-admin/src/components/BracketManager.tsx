...
import { getMatches, generateBracket, advanceWinner } from '../services/bracketService';
...
export const BracketManager: React.FC<BracketManagerProps> = ({ tournamentId, categoryId, areaId }) => {
  ...
  return (
    <div className="mt-6">
      ...
      {matches.length === 0 ? (
        <p className="text-gray-500 text-center py-8 bg-gray-50 rounded-lg">No bracket generated yet. Click generate to create matches from registered competitors.</p>
      ) : (
            ...
            {Array.from({ length: maxRounds }).map((_, i) => {
              ...
              return (
                <div key={r} className="flex flex-col space-y-4 w-64 justify-around">
                  ...
                  {roundMatches.map(m => (
                    <div key={m.id} className={`border rounded-md shadow-sm p-3 bg-white ${m.status === 'COMPLETED' ? 'opacity-70' : ''}`}>
                      ...
                      {/* Red Competitor */}
                      <div className={`flex justify-between items-center p-2 rounded mb-1 ${m.winnerId === m.redCompetitorId ? 'bg-red-100 font-bold' : 'bg-gray-50'}`}>
                        ...
                        {m.status !== 'COMPLETED' && m.redCompetitorId && m.redCompetitorId !== 'BYE' && (
                          <button 
                            onClick={() => handleAdvanceWinner(m.id, m.redCompetitorId, m.nextMatchId)}
                      ...
                      {/* Blue Competitor */}
                      <div className={`flex justify-between items-center p-2 rounded ${m.winnerId === m.blueCompetitorId ? 'bg-blue-100 font-bold' : 'bg-gray-50'}`}>
                         <div className="truncate w-3/4 flex items-center">
                        ...
                        {m.status !== 'COMPLETED' && m.blueCompetitorId && m.blueCompetitorId !== 'BYE' && (
                          <button 
                            onClick={() => handleAdvanceWinner(m.id, m.blueCompetitorId, m.nextMatchId)}
                            className="text-xs bg-gray-200 hover:bg-gray-300 px-2 py-1 rounded"
                            title="Advance as winner"
                          >
                            Win
