...
export const CategoryAdjuster: React.FC<CategoryAdjusterProps> = ({
  ...
  const _moveCompetitor = async (competitorId: string, newCategoryId: string) => {
    setProcessing(true)
    try {
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    if (!competitor || competitor.categoryId === newCategoryId) {
      return
    }
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="text-xl font-bold text-slate-100">Ajuste Manual de Categorías</h2>
          <p className="text-sm text-slate-400 mt-1">
            Reorganiza a los competidores arrastrándolos entre categorías, o utiliza el algoritmo de
            fusión automática.
          </p>
        </div>
        {!isReadOnly && (
      ...
      <Card padding="md" className="space-y-4 bg-slate-900/60 border-slate-800 backdrop-blur-xl">
        ...
        {loadingCategories || loadingCompetitors ? (
          <div className="py-12 text-center text-slate-500">Cargando tablero...</div>
              ...
              <div className="flex overflow-x-auto pb-6 pt-2 gap-4 snap-x">
                {filteredCategories.map((cat: Category) => (
                  <div key={cat.id} className="snap-start">
                    <CategoryDroppable
                      category={cat}
                      competitors={competitors.filter((c: Competitor) => c.categoryId === cat.id)}
                    />
                  </div>
                ))}
              </div>
                ...
                {activeDragCompetitor ? (
                  <div className="p-3 w-[280px] bg-slate-700 border-2 border-blue-500 rounded-lg shadow-2xl opacity-95 cursor-grabbing m-0">
                    ...
                    <div className="text-xs text-slate-400 mt-1">
                      ...
                      {activeDragCompetitor.weight ? `${activeDragCompetitor.weight}kg` : 'N/A kg'}
