...
export const CompetitorManager: React.FC<CompetitorManagerProps> = ({
  ...
  const generateMockCompetitors = async () => {
    ...
    const _countries = ['ARG', 'BRA', 'USA', 'CAN', 'CHI', 'URU', 'ESP']
    ...
    const mockCompetitors = Array.from({ length: amount }).map(() => {
      ...
      let age = 20
      if (randomCategory.ageGroup.includes('Micro')) {
        age = 6
      } else if (randomCategory.ageGroup.includes('Pre-Mini')) {
        age = 8
      } else if (randomCategory.ageGroup.includes('Mini')) {
        age = 10
      } else if (randomCategory.ageGroup.includes('Infantil')) {
        age = 12
      } else if (randomCategory.ageGroup.includes('Cadete')) {
        age = 14
      } else if (randomCategory.ageGroup.includes('Juvenil')) {
        age = 16
      } else if (randomCategory.ageGroup.includes('Adulto')) {
        age = 25
      } else if (randomCategory.ageGroup.includes('Senior')) {
        age = 40
      } else if (randomCategory.ageGroup.includes('Veterano')) {
        age = 50
      }
      ...
      if (age < 18) {
        weight = 20 + age * 2 + Math.floor(Math.random() * 10)
      }
    ...
    <div className="mt-6">
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-xl font-bold text-gray-900 dark:text-gray-100">
          {categoryId ? 'Competidores en esta categoría' : 'Todos los competidores'} (
          {competitors.length})
        </h2>
        {!isReadOnly && (
          <div className="flex space-x-2">
            ...
            <Button onClick={() => setIsFormOpen(true)} disabled={loading} variant="primary">
              Add Competitor
            </Button>
          </div>
        )}
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 px-4">
          <div className="bg-[#1e1e1e] border border-white/10 rounded-2xl shadow-2xl p-6 w-full max-w-md">
            ...
            <div className="flex justify-end space-x-3 mt-6">
              <Button onClick={() => setIsMockModalOpen(false)} variant="secondary">
                Cancelar
              </Button>
