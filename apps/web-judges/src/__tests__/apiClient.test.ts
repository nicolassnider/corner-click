...
describe('getDynamicApiUrl helper', () => {
  ...
  it('should fallback to port 4000 if loaded via Astro dev port 4321 on local address', () => {
    const configured = 'https://api.render.com'
    global.window.location.hostname = '192.168.1.10'
    ;(global.window.location as any).origin = 'http://192.168.1.10:4321'
    global.window.location.port = '4321'
  ...
  it('should return the configured production URL if loaded from Netlify production site', () => {
    const configured = 'https://api.render.com'
    global.window.location.hostname = 'corner-click-judges.netlify.app'
    ;(global.window.location as any).origin = 'https://corner-click-judges.netlify.app'
