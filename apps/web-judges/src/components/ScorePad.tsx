...
export default function ScorePad({
  ...
  const displayRedScore = calculateNetScore(
    redScore,
    redWarnings,
    redDeductions,
  );
  const displayBlueScore = calculateNetScore(
    blueScore,
    blueWarnings,
    blueDeductions,
  );
  ...
  useEffect(() => {
    ...
    if (
      ...
      const scoreRef = ref(
        database,
        `live_matches/${matchId}/scores/${cornerId}`,
      );
      set(scoreRef, {
        redScore,
        blueScore,
    ...
    >
      ...
      <div className="flex justify-between items-center px-4 py-2 bg-gray-950 border-b border-gray-800 z-30">
        <div className="flex items-center gap-2">
          <span className="text-gray-500 font-black tracking-widest uppercase text-[10px]">
            CORNER CLICK
          </span>
          {useLocal && (
            <span className="bg-amber-500/20 text-amber-400 border border-amber-500/30 text-[8px] font-black uppercase px-2 py-0.5 rounded tracking-widest animate-pulse">
              LOCAL FALLBACK
            </span>
          )}
        </div>
      ...
      <div className="flex-none bg-gray-900 border-b border-gray-800 shadow-xl z-20 pb-2 relative">
        <div className="flex items-center justify-between px-4 py-2">
          ...
          <div className="flex flex-col items-center w-1/3">
            <span className="text-rose-500 font-extrabold tracking-widest text-[10px] mb-1">
              ROJO
            </span>
            <span className="text-5xl font-black tabular-nums text-rose-500 drop-shadow-md">
              {displayRedScore}
            </span>
          ...
          <div className="text-gray-700 font-black text-xl tracking-widest opacity-50 w-1/3 text-center">
            VS
          </div>

          {/* Blue Score */}
          <div className="flex flex-col items-center w-1/3">
            <span className="text-indigo-400 font-extrabold tracking-widest text-[10px] mb-1">
              AZUL
            </span>
            <span className="text-5xl font-black tabular-nums text-indigo-400 drop-shadow-md">
              {displayBlueScore}
            </span>
          ...
          matchStatus !== MatchStatus.GOLDEN_POINT && (
            <div className="absolute inset-0 z-50 flex items-center justify-center bg-gray-950/80 backdrop-blur-sm">
              <div className="bg-yellow-500 text-black px-6 py-4 rounded-2xl shadow-2xl flex flex-col items-center border-4 border-yellow-400 animate-pulse">
                ...
                <p className="text-sm font-bold uppercase opacity-80 mt-1">
                  Status: {matchStatus}
                </p>
              </div>
            </div>
